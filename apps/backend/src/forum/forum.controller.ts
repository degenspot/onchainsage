...
export class ForumController {
  ...
  async createPost(@Body() postData: { author: string; content: string }) {
    try {
      this.logger.log(`Received request to create post from author: ${postData.author}`);
      
      // Validate input
      if (!postData.author || !postData.content) {
        throw new HttpException('Author and content are required', HttpStatus.BAD_REQUEST);
      }
      
      // Call StarknetService to create post on-chain
      const txHash = await this.starknetService.createPost(postData.author, postData.content);
  ...
  async votePost(@Body() voteData: { postId: string; vote: number }) {
    try {
      ...
      if (!voteData.postId || voteData.vote === undefined) {
        throw new HttpException('Post ID and vote are required', HttpStatus.BAD_REQUEST);
      }
      
      // Validate vote value
      if (voteData.vote !== 1 && voteData.vote !== -1) {
        throw new HttpException('Vote must be 1 (upvote) or -1 (downvote)', HttpStatus.BAD_REQUEST);
      }
      
      // Call StarknetService to vote on post on-chain
      const txHash = await this.starknetService.votePost(voteData.postId, voteData.vote);
  ...
  async awardBadge(@Body() badgeData: { user: string; badge: string }) {
    try {
      this.logger.log(`Received request to award badge ${badgeData.badge} to user: ${badgeData.user}`);
      
      // Validate input
      if (!badgeData.user || !badgeData.badge) {
        throw new HttpException('User address and badge are required', HttpStatus.BAD_REQUEST);
      }
      
      // Call StarknetService to award badge on-chain
      const txHash = await this.starknetService.awardBadge(badgeData.user, badgeData.badge);
