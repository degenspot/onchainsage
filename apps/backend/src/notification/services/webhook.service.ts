...
export class WebhookService {
  ...
  async findWebhook(id: string, userId: string): Promise<WebhookRegistration> {
    ...
    if (!webhook) {
      throw new NotFoundException(
        `Webhook with id ${id} not found for this user`,
      );
    }
  ...
  async requestWebhookVerification(
    ...
    try {
      ...
      return {
        message: `Failed to send verification request: ${error.message}`,
      };
    }
  }
  ...
  async getWebhookDeliveryHistory(
    ...
    const [deliveries, total] =
      await this.deliveryRecordRepository.findAndCount({
        where: {
    ...
    try {
      ...
      if (webhook.failureCount <= retryStrategy.maxRetries) {
        const delay =
          ...
          Math.pow(retryStrategy.backoffMultiplier, webhook.failureCount - 1);
        ...
        console.error(
          `Exhausted retries for webhook ${webhook.id} for event ${eventName}`,
        );
  ...
  public async deliverEvent(
    ...
    if (webhook.configuration.secretKey) {
      const signature = this.generateSignature(
        eventPayload,
        webhook.configuration.secretKey,
      );
    ...
    const deliveryRecord =
      await this.deliveryTrackingService.trackDeliveryStart(
    ...
    try {
      ...
      if (method === 'POST') {
        ...
        response = await firstValueFrom(
          this.httpService.put(webhook.url, eventPayload, { headers, timeout }),
        );
      } else {
  ...
  private checkRateLimit(webhookId: string): boolean {
    ...
    const webhook = this.webhookRepository.findOne({
      where: { id: webhookId },
    });
    const maxPerMinute =
      webhook['configuration']?.rateLimit?.maxPerMinute || 60;
