...
} from '../interfaces/engagement-analytics.interface';
...
export class EngagementAnalyticsService {
  ...
  async getEngagementAnalytics(
    ...
    const totalEngagements = Object.values(engagementsByType).reduce(
      (sum, count) => sum + count,
      0,
    );
   ...
   */
  private async getEngagementCountsByType(
    query: EngagementQueryDto,
    ...
    if (query.contentId) {
      ...
      const timeSeries =
        await this.engagementRepository.getTimeSeriesData(query);
  ...
  private calculateSentimentScore(
    ...
    const positive =
      weightedScores[EngagementType.LIKE] +
      weightedScores[EngagementType.SHARE];
