...
export class EngagementCacheService {
  ...
  async invalidateContentEngagement(contentId: string): Promise<void> {
    try {
      ...
      this.logger.debug(`Invalidated ${keys.length} cache entries for content ${contentId}`);
    } catch (error) {
      this.logger.error(`Error invalidating content cache for ${contentId}: ${error.message}`);
    }
  }
