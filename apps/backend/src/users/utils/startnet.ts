import { Account, RpcProvider, ec, typedData, hash } from 'starknet';
...
export async function verifyStarknetMessage(
  ...
  try {
    if (!walletAddress || !message || !signature || signature.length !== 2) {
      throw new Error("Invalid parameters: walletAddress, message, or signature is missing.");
    }
    ...
    const provider = new RpcProvider({ 
        nodeUrl: process.env.STARKNET_NODE_URL || 'https://starknet-mainnet.infura.io/v3/YOUR_INFURA_PROJECT_ID'
      });
      
    //Account instance (Verification does not require private key)
    const account = new Account(provider, walletAddress, ec.starkCurve.getPublicKey(walletAddress));
    ...
    const starkSignature: string[] = [signature[0].toString(), signature[1].toString()];
